  ...
  return (
    <WorkspaceLayout title="智能体中心"
      rightPanel={selectedSkill ? (() => { const s = knowledgeSkills.find(k => k.id === selectedSkill)!; return (
        <div className="p-4">
          <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-[16px]">Skill 详情</h3><button onClick={()=>setSelectedSkill(null)} className="text-gray-400 hover:text-gray-600"><XCircle size={16}/></button></div>
          <div className="flex items-center gap-3 mb-4"><span className={clsx('tag', s.color)}>{s.icon}</span><div><div className="font-semibold text-[16px]">{s.name}</div><div className="text-[16px] text-gray-400">{s.source}</div></div></div>
          <p className="text-[16px] text-gray-600 mb-4">{s.desc}</p>
          <div className="mb-4"><h4 className="text-[16px] font-semibold text-gray-400 uppercase tracking-wide mb-2">调用示例</h4>
            {s.examples.map((ex,i)=><div key={i} className="p-3 bg-gray-50 rounded text-[16px] text-gray-700 mb-1.5">{ex}</div>)}
          </div>
          <div className="mb-4"><h4 className="text-[16px] font-semibold text-gray-400 uppercase tracking-wide mb-2">绑定 Agent</h4><div className="text-[16px] text-gray-600 bg-gray-50 p-3 rounded">{s.agent}</div></div>
          <div className="p-3 bg-amber-50 border border-amber-100 rounded text-[16px] text-amber-700">⚠ Phase 2: 对接飞书知识库 API 后，知识检索将返回真实文档内容。</div>
        </div>
      )})() : (
        <div className="p-4"><h3 className="text-[16px] font-semibold text-gray-400 uppercase tracking-wide mb-3"><Zap size={14} className="inline mr-1"/>快速触发</h3>
          <div className="space-y-2">{knowledgeSkills.flatMap(s=>s.examples.slice(0,1).map((ex,i)=>
            <button key={`${s.id}-${i}`} onClick={()=>setSelectedSkill(s.id)} className="w-full text-left p-3 border border-gray-100 rounded-md text-[16px] text-gray-600 hover:bg-gray-50 transition-colors">
    ...
    >
      <div className="p-6 space-y-6">
        <div>
          <h2 className="text-[16px] font-semibold text-gray-400 uppercase tracking-wide mb-3">在线 Agent</h2>
            ...
            {agents.map(agent => { const s = statusConfig[agent.status]; if (!s) return null; return (
              <div key={agent.id} className="proto-card p-4 animate-in">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3"><Bot size={24} className="text-brand-500"/><div><div className="font-semibold text-[16px]">{agent.name}</div><div className="text-[16px] text-gray-400">{agent.type} · Level {agent.level}</div></div></div>
                  <span className={clsx('tag flex items-center gap-1', s.cls)}>{s.icon}{s.label}</span>
                </div>
                <p className="text-[16px] text-gray-500 mb-3">{agent.desc}</p>
                <div className="border-t border-gray-100 pt-3 grid grid-cols-2 gap-2 text-[16px] text-gray-500">
        ...
        <div>
          <h2 className="text-[16px] font-semibold text-gray-400 uppercase tracking-wide mb-3">Knowledge Agent · Skills</h2>
          <div className="grid grid-cols-2 gap-4">
            {knowledgeSkills.map(sk => (
              <button key={sk.id} onClick={()=>setSelectedSkill(sk.id)} className={clsx('proto-card p-4 text-left hover:border-brand-300 transition-all cursor-pointer', selectedSkill===sk.id && 'ring-2 ring-brand-100 border-brand-300')}>
                <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><span className={clsx('tag', sk.color)}>{sk.icon}</span><span className="font-semibold text-[16px]">{sk.name}</span></div><ChevronRight size={14} className="text-gray-300"/></div>
                <p className="text-[16px] text-gray-500 mb-2">{sk.desc}</p>
                <div className="flex items-center gap-2 text-[16px] text-gray-400"><span>{sk.source}</span><span>·</span><span>{sk.agent}</span></div>
              </button>
            ))}
          </div>
        </div>
