          ...
          <div className="proto-card-accent p-4">
            ...
            <div className="space-y-2">
              {risks.slice(0, 3).map((r, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-md bg-gray-50 text-xs">
                  <div><div className="font-medium text-gray-800">{r.customer}</div><div className="text-gray-400 text-3xs">信用 {r.credit}</div></div>
            ...
            <tbody>
              {risks.map((r, i) => (
                <tr key={i} className="cursor-pointer">
                  ...
                  <td className="text-gray-600 text-2xs">{r.action}</td>
                </tr>
              ))}
            </tbody>
          </table>
