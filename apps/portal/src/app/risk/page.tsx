...
export default function RiskCenterPage() {
  ...
  const riskColors: Record<string, string> = {
    L4_High: 'bg-red-100 text-red-700',
    L3_Warning: 'bg-orange-100 text-orange-700',
    L2_Watch: 'bg-yellow-100 text-yellow-700',
    L1_Low: 'bg-green-100 text-green-700',
  };
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center h-12 px-6 bg-white border-b border-gray-200 shrink-0">
          <h1 className="text-lg font-semibold text-gray-900">AR Risk Center — 风控中心</h1>
        </header>
          ...
          <main className="flex-1 overflow-y-auto p-6">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-left text-gray-500 font-medium">
                  <tr>
                    ...
                    <th className="px-4 py-3">未回款金额</th>
                  ...
                  {arRisks.map((r, i) => (
                    <tr key={i} className="border-t border-gray-100 hover:bg-gray-50 cursor-pointer">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-1 rounded font-medium ${riskColors[r.risk]}`}>
                          {r.risk}
                        </span>
                      </td>
            ...
            <div className="mt-6">
              <AgentSuggestionCard
                ...
                evidence={['逾期金额 $380,000', '最近一次回款: 95天前', '信用评分: 从B→C 持续下降']}
                sourceId="AR-RISK-001"
                generatedAt="2026-06-02 11:00"
                confidenceScore={91}
                suggestedActions={[
                  { label: '暂停发货', risk: 'high' },
                  { label: '发送法务催收函', risk: 'medium' },
                  { label: '启动法务程序', risk: 'high' },
                ]}
              ...
              {arRisks.slice(0, 3).map((r, i) => (
                <div key={i} className="p-3 border border-gray-100 rounded">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded ${riskColors[r.risk]}`}>{r.risk}</span>
                  </div>
        ...
        <BottomCommandBar placeholder="查询客户风险、生成催收建议..." />
      </div>
    </div>
  );
}
