  ...
  return (
    <div className="proto-card-accent p-4 text-sm">
      ...
      <div className="flex gap-3 mt-3 pt-3 border-t border-gray-100">
        {p.sourceURL && <button className="flex items-center gap-1 text-base text-brand-600 hover:text-brand-700 transition-colors"><ExternalLink size={12} />查看来源</button>}
        {p.onViewAudit && <button onClick={p.onViewAudit} className="text-base text-gray-400 hover:text-gray-600 transition-colors">审计轨迹</button>}
      </div>
    </div>
  );
}
