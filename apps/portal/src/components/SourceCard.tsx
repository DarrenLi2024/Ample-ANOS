...
export function SourceCard(p: SourceCardProps) {
  ...
  return (
    <div className="proto-card-accent p-4 text-[16px]">
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex justify-between"><span className="text-gray-400">原始时间</span><span className="font-mono text-[16px]">{p.eventTime}</span></div>
        <div className="flex justify-between"><span className="text-gray-400">采集时间</span><span className="font-mono text-[16px]">{p.capturedAt}</span></div>
      ...
      <div className="flex gap-3 mt-3 pt-3 border-t border-gray-100">
        {p.sourceURL && <button className="flex items-center gap-1 text-[16px] text-brand-600 hover:text-brand-700 transition-colors"><ExternalLink size={12} />查看来源</button>}
        {p.onViewAudit && <button onClick={p.onViewAudit} className="text-[16px] text-gray-400 hover:text-gray-600 transition-colors">审计轨迹</button>}
      </div>
    </div>
  );
}
