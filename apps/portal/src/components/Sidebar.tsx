      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {Array.from(sections.entries()).map(([section, items]) => (
          <div key={section} className="mb-3">
            {!collapsed && <div className="px-5 py-1.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{sectionLabels[section]}</div>}
            {items.map(item => {
              const active = isActive(item);
              return (
                <button key={item.id} onClick={() => router.push(item.href)}
                  className={clsx('w-full flex items-center gap-3 mx-2 px-3 py-2 rounded-md text-sm transition-all duration-150',
