    ...
    <div className="proto-card-accent animate-in">
      ...
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-semibold text-lg text-gray-900">{agentName}</span>
            <span className={clsx('tag ml-2', typeStyles[agentType])}>{agentType}</span>
          </div>
        </div>
        <span className="text-lg text-gray-400">{generatedAt}</span>
      </div>

      {/* Body */}
      <div className="p-5">
        <p className="text-lg text-gray-800 leading-relaxed mb-3">{conclusion}</p>
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-lg font-medium text-gray-500">{confidenceScore}%</span>
        </div>

        {/* Evidence */}
        {evidence.length > 0 && (
          <>
            <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-1 text-lg text-brand-600 hover:text-brand-700 mb-2 transition-colors">
              {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
              {evidence.length} 条依据
            </button>
            {expanded && (
              <ul className="space-y-1.5 mb-3">{evidence.map((e, i) => (
                <li key={i} className="text-lg text-gray-500 pl-3 border-l-2 border-brand-200 py-0.5">{e}</li>
              ))}</ul>
            )}
          </>
        )}
        ...
        {suggestedActions.length > 0 && (
          <div className="space-y-2 mb-3">
            {suggestedActions.map((a, i) => (
              <div key={i} className="flex items-center justify-between text-lg border border-gray-100 rounded-md px-3 py-2.5 bg-gray-50/50">
        ...
        {requiresApproval && (
          <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-medium text-amber-700">需要人工确认</span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setVoted('up')}
                className={clsx('btn-outline !py-1.5 !px-3 !text-lg flex items-center gap-1.5', voted === 'up' && '!border-green-300 !text-green-700 !bg-green-50')}>
                <ThumbsUp size={14} />确认
              </button>
              <button onClick={() => setVoted('down')}
                className={clsx('btn-outline !py-1.5 !px-3 !text-lg flex items-center gap-1.5', voted === 'down' && '!border-red-300 !text-red-700 !bg-red-50')}>
        ...
        {!sourceId && <p className="text-lg text-red-500 mt-3">⚠ 缺少来源引用</p>}
      </div>
    </div>
  );
}
